...
// Async local storage for request context (request ID, trace ID)
...
// Base logger options
function getLoggerOptions(moduleName: string): LoggerOptions {
...
// Types for request logging middleware
...
export function requestLoggingMiddleware() {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const traceId = req.headers["x-opik-trace-id"] as string | undefined;
    ...
    res.on("finish", () => {
      ...
      apiLogger[logLevel](
        { request: requestInfo, response: responseInfo, requestId, ...(traceId && { opikTraceId: traceId }) },
        `${req.method} ${req.path} ${res.statusCode} - ${responseInfo.responseTime}ms`
      );
    });
...
export function setOpikTraceId(traceId: string): void {
  const ctx = requestContext.getStore();
  if (ctx) {
    ctx.traceId = traceId;
  }
...
export function getOpikTraceId(): string | undefined {
  return requestContext.getStore()?.traceId;
}

// Export types for external use
export type { RequestContext, RequestLogInfo, ResponseLogInfo };
