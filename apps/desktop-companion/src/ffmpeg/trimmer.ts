 ...
 */
export function configureFfmpeg(ffmpegPath?: string): void {
  if (ffmpegPath) {
...
export function calculateBufferOffsets(params: {
  ...
  actualBufferDuration?: number;
}): { startOffset: number; endOffset: number; clipDuration: number } {
  const {
    t0,
    t1,
    sessionStartedAt,
  ...
  const endOffset = Math.min(
    bufferDuration,
    (clipEndAbsolute - bufferStartAbsolute) / 1000
  );

  // Validate offsets
  if (startOffset >= bufferDuration) {
    throw new FFmpegError(
      'Clip start time is beyond the replay buffer. The moment may have occurred before the buffer started.',
      'INVALID_TIMESTAMPS',
      { t0, t1, startOffset, endOffset, bufferDuration }
    );
  }

  if (endOffset <= 0) {
    throw new FFmpegError(
      'Clip end time is before the replay buffer starts. This should not happen.',
      'INVALID_TIMESTAMPS',
      { t0, t1, startOffset, endOffset, bufferDuration }
    );
  }
  ...
  if (startOffset >= endOffset) {
    throw new FFmpegError(
      'Invalid clip timestamps: start must be before end',
      'INVALID_TIMESTAMPS',
      { t0, t1, startOffset, endOffset }
    );
  }
      ...
      .on('error', (err, stdout, stderr) => {
        ...
        reject(
          new FFmpegError(
            ...
            'TRIM_FAILED',
            { inputPath, outputPath, startOffset, duration, stderr }
          )
        );
      })
      .on('end', () => {
 ...
 * ```
 */
export async function trimClip(input: TrimClipInput): Promise<TrimClipResult> {
  const {
    replayBufferPath,
    t0,
    t1,
    ...
    format = 'mp4',
    replayBufferSavedAt = Date.now(),
    sessionStartedAt = Date.now() - 300000, // Default: assume session started 5 min ago
    replayBufferSeconds = 300,
  } = input;
  ...
  if (!fs.existsSync(replayBufferPath)) {
    throw new FFmpegError(
      `Replay buffer file not found: ${replayBufferPath}`,
  ...
  if (t0 < 0 || t1 < 0) {
    throw new FFmpegError(
      'Clip timestamps must be non-negative',
      'INVALID_TIMESTAMPS',
      { t0, t1 }
    );
  }
  ...
  if (t0 >= t1) {
    throw new FFmpegError(
      'Clip start time must be before end time',
      'INVALID_TIMESTAMPS',
      { t0, t1 }
    );
  }
  ...
  try {
    ...
    throw new FFmpegError(
      ...
      'OUTPUT_DIR_ERROR',
      { clipsDir, thumbnailsDir, error: err }
    );
  }
 ...
 */
export async function trimClipDirect(
  inputPath: string,
  outputDir: string,
  artifactId: string,
  startOffset: number,
  ...
  if (duration <= 0) {
    throw new FFmpegError(
      'Invalid offsets: end must be after start',
      'INVALID_TIMESTAMPS',
