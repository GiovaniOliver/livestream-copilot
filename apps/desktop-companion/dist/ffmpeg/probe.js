 ...
 */
export function configureFfprobe(ffprobePath) {
    if (ffprobePath) {
...
export async function probeVideo(filePath) {
    ...
    if (!fs.existsSync(filePath)) {
        throw new FFmpegError(`Input file not found: ${filePath}`, 'INPUT_FILE_NOT_FOUND', { filePath });
    }
    return new Promise((resolve, reject) => {
        ffmpeg.ffprobe(filePath, (err, metadata) => {
            if (err) {
                reject(new FFmpegError(`Failed to probe video: ${err.message}`, 'PROBE_FAILED', { filePath, error: err }));
                return;
            }
            try {
                const videoStream = metadata.streams.find((s) => s.codec_type === 'video');
                if (!videoStream) {
                    reject(new FFmpegError('No video stream found in file', 'PROBE_FAILED', { filePath, streams: metadata.streams.map((s) => s.codec_type) }));
                    return;
                }
                ...
                reject(new FFmpegError(`Failed to parse video metadata: ${parseError}`, 'PROBE_FAILED', { filePath, metadata, error: parseError }));
            }
        });
    });
}
    ...
    return new Promise((resolve) => {
        ...
        ffmpeg.ffprobe('/nonexistent/test/file.mp4', (err) => {
            if (err) {
                ...
                if (errMessage.includes('ffprobe') &&
